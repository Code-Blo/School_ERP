...
// ... (uploadAssignment function remains the same)
export const uploadAssignment = async (req, res) => {
  try {
    if (req.role !== "teacher") {
      throw new ApiError(403, "Only teachers can upload assignments");
...
// ✅ FIX: The submitAssignment function is now corrected
export const submitAssignment = async (req, res) => {
  try {
    if (req.role !== "student") {
      throw new ApiError(403, "Only students can submit assignments");
    ...
    // The file path is now relative to the 'uploads' directory
    const fileUrl = `/submissions/${req.file.filename}`;

    const submission = new StudentSubmission({
      assignmentId: req.body.assignmentId,
      studentId: req.user._id,
      // This is the critical fix: get adminId from the logged-in user
...
// ... (getAssignmentsByClass and getSubmissionsByClassAndSubject remain the same)
export const getAssignmentsByClass = async (req, res) => {
  const { className } = req.params;
  try {
    const assignments = await Assignment.find({
