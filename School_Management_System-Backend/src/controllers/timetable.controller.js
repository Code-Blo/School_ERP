...
// ✅ **FIXED: Get Timetable by Class (Now works for Students/Teachers)**
export const getTimetableByClassName = async (req, res) => {
  try {
    const { className } = req.params;
    ...
    // Determine the correct adminId based on the logged-in user's role
    const adminId = req.user.role === 'admin' ? req.user._id : req.user.adminId;

    if (!adminId) {
        return res.status(401).json({ message: 'Could not determine admin context for this user.' });
    }
...
// ✅ Get all timetables for the logged-in admin
export const getAllTimetablesForAdmin = async (req, res) => {
  try {
    const timetables = await Timetable.find({ adminId: req.user._id });
    res.status(200).json(timetables);
