...
const Header = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  const getProfileImageUrl = () => {
    if (user && user.profileImage) {
      // Always prepend the backend URL to the relative path
      return `${import.meta.env.VITE_BACKEND_URL}${user.profileImage}`;
    }
  ...
  return (
    <div className="flex justify-between items-center mb-6 px-4 py-4 bg-white/5 backdrop-blur-md rounded-xl border border-white/10 shadow-md">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Profile Image / Initial */}
        <div
          className="w-9 h-9 rounded-full overflow-hidden bg-gray-700 flex items-center justify-center text-white text-sm font-bold cursor-pointer"
          onClick={handleProfileClick}
        >
          {profileImageUrl ? (
            <img
              // ✅ **FIX: Use the constructed URL**
              src={profileImageUrl}
              alt="Profile"
              className="w-full h-full object-cover"
