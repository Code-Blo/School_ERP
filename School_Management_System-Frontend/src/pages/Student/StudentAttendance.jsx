...
const StudentAttendance = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setStudent(res.data);
        // ✅ FIX: The profile endpoint returns 'className', not 'class'
        setSelectedClass(res.data.className); 
      } catch (err) {
        console.error("Error loading profile:", err);
        setError("Failed to load profile data");
      }
  ...
  const loadAttendance = async () => {
    // ✅ FIX: Use the correct ID field from the student object ('id' or '_id')
    ...
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/attendance/student/${studentId}?year=${academicYear}`,
        { withCredentials: true }
      );
      setAttendance(Array.isArray(res.data?.records) ? res.data.records : []);
    } catch (err) {
  ...
  useEffect(() => {
    // ✅ FIX: Check for student object before loading
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/leaves/submit`,
        {
          studentId: studentId,
          studentName: student.name,
          class: student.className, // ✅ FIX: Use 'className'
          rollNo: student.rollNo,
          fromDate: leaveData.fromDate,
          toDate: leaveData.toDate,
          reason: leaveData.reason
