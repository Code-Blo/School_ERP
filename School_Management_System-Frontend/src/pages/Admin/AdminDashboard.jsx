...
const AdminDashboard = () => {
  ...
  // ✅ 1. Add state to hold dashboard stats
  ...
  // ✅ 2. Fetch both notices and stats on component load
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch notices
        const noticeRes = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/notices`, { withCredentials: true });
        setNotices(noticeRes.data || []);
        
        // Fetch dashboard stats
        const statsRes = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/dashboard/stats`, { withCredentials: true });
  ...
  // ✅ 3. Update cardData to use the new state
  const cardData = [
    {
      label: 'Total Students',
    ...
    },
    {
      ...
      count: `₹${new Intl.NumberFormat('en-IN').format(stats.totalFeeCollection)}`,
      icon: <FaMoneyBillWave className="text-4xl text-white drop-shadow-lg" />,
      note: 'Total revenue',
      link: '/dashboard/admin/finance',
    },
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white relative overflow-hidden">
      ...
      <div className="flex-1 p-6 space-y-10">
        ...
        {/* ... (rest of your JSX for the Notice Board remains the same) ... */}
        <div className="bg-white/5 p-6 rounded-2xl shadow-md backdrop-blur-md border border-white/10 mt-10 relative">
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-2xl font-bold">📌 Notice Board</h3>
            <button
